...
| File | Purpose |
|------|---------|
| `ingest_papers_embeddings.py` | Databricks notebook: Ingests research papers from OpenAlex & Semantic Scholar, chunks text, generates 384-d embeddings with `all-MiniLM-L6-v2`, and batch-persists vectors to Lakebase pgvector with HNSW index verification. |
...
9. **Batch Vector Encoding** (`sentence-transformers/all-MiniLM-L6-v2` with `normalize_embeddings=True`)
10. **Batch Vector Upsert** (`psycopg2.extras.execute_batch` with `%s::vector(384)`)
...
### 1. Sliding Window Character Chunking (`CHUNK_SIZE=800`, `CHUNK_OVERLAP=100`)
* **Context Preservation:** Abstract texts are divided into overlapping chunks. The 100-character overlap prevents key semantic clauses from being split across chunk boundaries.
...
### 2. Normalized Vectors with `all-MiniLM-L6-v2` (384 Dimensions)
...
* **Cost & Speed:** MiniLM offers ~14,000 sentences/sec throughput with minimal latency, saving compute costs on Databricks clusters while matching 90%+ quality of 768-dim models.
