...
| File | Purpose |
|------|---------|
| `auth.py` | Resolves the Databricks-injected `X-Forwarded-Email` header (or the seeded demo user in local dev) to a `users` row once per request, exposed on Flask's `g.user` / `g.user_id`. |
...
* Databricks Apps run behind an authenticating proxy that injects `X-Forwarded-Email` on every request — the app never sees a password and needs no session store.
* `auth.py` upserts the user on first sight (`get_or_create_user`), so a brand-new Databricks user gets a row automatically.
* Local development has no proxy, so the middleware falls back to `demo@research-copilot.dev` — the *same* identity `mcp_server/middleware/request_context.py` defaults to, so the agent and the dashboard read and write one account.
